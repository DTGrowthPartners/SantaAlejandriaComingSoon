...
function canManageRooms(role: UserRole) {
...
export async function createRoomAction(
  ...
  if (!canManageRooms(session.role)) return fail("Solo Admin/Gerencia gestionan habitaciones.");
...
export async function updateRoomAction(
  ...
  await prisma.room.update({
    where: { id },
    data: { name: d.name, type: d.type || null, capacity: d.capacity, sortOrder: d.sortOrder },
  });
  revalidate();
  return OK;
}
